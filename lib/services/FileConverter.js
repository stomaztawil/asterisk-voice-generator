...
export class FileConverter {
    ...
    async convertFile(inputPath, outputPath, format) {
        ...
        if (!preset) throw new Error(`Unsupported format: ${format}`);
    ...
    async convertToG729(inputPath, outputPath) {
        try {
            ...
            await this.executeCommand(step1, 'FFmpeg G729 (step 1)');
            await this.executeCommand(step2, 'Sox G729 (step 2)');
    ...
    async executeCommand(command, context) {
        try {
            ...
            if (stderr) console.warn(`[${context}] Warning: ${stderr}`);
        } catch (error) {
