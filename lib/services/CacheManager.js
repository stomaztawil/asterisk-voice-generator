...
export class CacheManager {
    ...
    async load() {
        try {
            if (await fs.pathExists(this.cachePath)) {
                ...
                console.log(`Cache carregado: ${Object.keys(this.cache).length} entradas`);
            }
        } catch (error) {
            console.warn('Erro ao carregar cache, iniciando novo:', error.message);
        }
    }
    ...
    async saveIfChanged() {
        ...
        try {
            ...
            this.changed = false; // Resetar flag após salvar
        } catch (error) {
